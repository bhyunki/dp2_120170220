...
pthread_mutex_t		count_mutex, my_mutex=PTHREAD_MUTEX_INITIALIZER;
...
//         pthread_rwlock_rdlock(&rwlock);
	 my_rdlock(&my_mutex, &cv, &count);
         Member(val);
//         pthread_rwlock_unlock(&rwlock);
	 my_rdunlock(&my_mutex, &cv, &count);
         my_member_count++;
      } else if (which_op < search_percent + insert_percent) {
//         pthread_rwlock_wrlock(&rwlock);
	 my_wrlock(&my_mutex, &cv);
         Insert(val);
//         pthread_rwlock_unlock(&rwlock);
	 my_wrunlock(&my_mutex, &cv);
         my_insert_count++;
      } else { /* delete */
//         pthread_rwlock_wrlock(&rwlock);
	 my_wrlock(&my_mutex, &cv);
